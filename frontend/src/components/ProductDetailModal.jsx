  ...
  const handleSaveImages = async () => {
    if (newImages.length > 0 && selectedProduct) {
      ...
      try {
        ...
        console.log('Sending to API:', { thumbnails: updatedThumbnails }); // Debug
        const response = await fetch(
          `${import.meta.env.VITE_BE_URL}/api/product/${selectedProduct.id}`,
          {
            method: 'PUT',
        ...
        const updatedProduct = await response.json();
        console.log('API response:', updatedProduct); // Debug
        // Cập nhật selectedProduct với dữ liệu từ API
        setSelectedProduct((prev) => ({
          ...prev,
          thumbnails: updatedProduct.thumbnails || updatedThumbnails,
        }));
        // Cập nhật products list để đồng bộ
        setSelectedProduct((prev) => {
