...
class PaymentController {
    ...
    async handlePaymentCallback(req, res) {
        try {
            ...
            if (result) {
                res.redirect(`${process.env.FRONTEND_URL}/business-registration?payment=success&payment_id=${result.data.payment_id}`);
            }else {
                res.redirect(`${process.env.FRONTEND_URL}/business-registration?payment=failed`);
            }
        } catch (err) {
