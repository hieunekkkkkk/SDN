...
import { getCurrentUserId } from '../../utils/useCurrentUserId'; // Giả sử file này tồn tại
...
const MyBusinessPage = () => {
  ...
  useEffect(() => {
    const fetchBusinessData = async () => {
      ...
      const userId = getCurrentUserId(); // Sử dụng getCurrentUserId thay vì user.id
      ...
      try {
        ...
        console.log('Businesses data:', businesses); // Debug dữ liệu từ API
        ...
        // Bước 2: Filter doanh nghiệp theo user_id
        const userBusiness = businesses.find((b) => b.user_id === userId);
        console.log('User Business found:', userBusiness); // Debug kết quả filter
        if (!userBusiness) {
          const availableUserIds = businesses.map((b) => b.user_id).join(', ');
          setError(
            `Không tìm thấy doanh nghiệp nào cho user với ID: ${userId}. Các user_id trong database: ${availableUserIds}. Vui lòng kiểm tra hoặc liên hệ admin để gán doanh nghiệp.`
          );
          setLoading(false);
          return;
        }
        ...
        // Bước 3: Fetch business details, products, and feedbacks in parallel
